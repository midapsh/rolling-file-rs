...
#[derive(Debug)]
pub struct RollingFileAppenderGeneric<RC, Tz>
where
    RC: RollingCondition<Tz>,
    Tz: TimeZone,
{
    condition: RC,
...
impl<RC, Tz> RollingFileAppenderGeneric<RC, Tz>
where
    RC: RollingCondition<Tz>,
    Tz: TimeZone,
{
    ...
    pub fn new<P>(
        ...
        max_files: usize,
        time_zone: Tz,
    ) -> io::Result<RollingFileAppenderGeneric<RC, Tz>>
    where
        P: AsRef<Path>,
    {
        let mut rfa = RollingFileAppenderGeneric {
            condition,
    ...
    pub fn write_with_datetime(&mut self, buf: &[u8], now: &DateTime<Tz>) -> io::Result<usize> {
    ...
    pub(crate) fn now(&self) -> DateTime<Tz> {
        SystemClock::now(self.time_zone.clone())
    }
}

impl<RC, Tz> io::Write for RollingFileAppenderGeneric<RC, Tz>
where
    RC: RollingCondition<Tz>,
    Tz: TimeZone,
{
